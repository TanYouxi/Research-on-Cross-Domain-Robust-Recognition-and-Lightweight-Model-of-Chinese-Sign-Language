...
RGB video -> MobileNetV3 frame encoder -> TCN -> CTC

Included by default:
- best/latest checkpoint saving
- optimizer + scheduler state saving
- resume training
- dev-based best model selection
...
CONFIG = {
    ...
    "output_dir": r"E:\CE-CSL\CE-CSL\experiments\m0_rgb",
...
def build_vocab_from_manifests(manifest_paths: List[str], gloss_key: str, vocab_path: str) -> CTCVocab:
    ...
    for i, tok in enumerate(sorted(token_set), start=1):
        token_to_id[tok] = i
    ensure_dir(str(Path(vocab_path).parent))
...
def load_or_build_vocab(cfg: Dict[str, Any]) -> CTCVocab:
    if os.path.exists(cfg["vocab_path"]):
        ...
        return CTCVocab(token_to_id)
    return build_vocab_from_manifests(
        [cfg["train_manifest"], cfg["dev_manifest"], cfg["test_manifest"]],
        cfg["gloss_key"],
        cfg["vocab_path"],
    )


class CECSLRGBDataset(Dataset):
    ...
    def _read_video_frames(self, video_path: str) -> torch.Tensor:
        ...
        cap.release()
        if len(selected) == 0:
            raise RuntimeError(f"No selected frames: {video_path}")
...
class M0RGBCTC(nn.Module):
    def __init__(self, vocab_size: int, cfg: Dict[str, Any]):
        super().__init__()
        self.frame_encoder = FrameEncoderMobileNetV3Small()
        self.temporal = TemporalConvNet(
            cfg["frame_feature_dim"], cfg["tcn_hidden_dim"], cfg["tcn_num_layers"], cfg["tcn_kernel_size"], cfg["tcn_dropout"]
        )
...
def train_one_epoch(model, loader, criterion, optimizer, device, blank_id, grad_clip, log_interval=50) -> EpochResult:
    model.train()
    t0 = time.time()
    total_loss = 0.0
    total_batches = 0
    ...
    return EpochResult(total_loss / max(total_batches, 1), wer, time.time() - t0)
...
def main():
    ...
    train_ds = CECSLRGBDataset(cfg["train_manifest"], vocab, cfg)
    dev_ds = CECSLRGBDataset(cfg["dev_manifest"], vocab, cfg)
    test_ds = CECSLRGBDataset(cfg["test_manifest"], vocab, cfg)
    ...
    if cfg["test_after_training"]:
        ...
        print("\n" + "=" * 80)
        print("[TEST RESULT]")
        print(f"loss={test_res.loss:.4f} | WER={test_res.wer:.4f}")
        print("=" * 80)
        save_json({

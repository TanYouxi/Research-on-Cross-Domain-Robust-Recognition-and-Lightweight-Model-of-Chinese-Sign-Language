# -*- coding: utf-8 -*-
"""
M3: RGB + ST-GCN Skeleton + Perception Alignment + CLIP Semantic Alignment for CE-CSL
AutoDL-ready version

Design goals:
- Keep the M2 training/inference pipeline stable
- Add CLIP-based text semantic alignment as an incremental module for M3
...
CONFIG = {
    ...
    "experiment_name": "m3_rgb_skel_align_clip_stgcn",
    "output_dir": f"{DATA_ROOT}/experiments/m3_rgb_skel_align_clip_stgcn",
    "seed": 42,
    ...
    "num_workers": 8,
    "pin_memory": True,
    "resume_path": f"{DATA_ROOT}/experiments/m3_rgb_skel_align_clip_stgcn/checkpoints/latest.pt",
    "learning_rate": 1e-4,
    "weight_decay": 1e-4,
    "grad_clip": 5.0,
    "use_scheduler": True,
...
class M3RGBSkeletonCLIPCTC(nn.Module):
    def __init__(self, vocab_size: int, cfg: Dict[str, Any]):
        ...
        self.text_encoder = CLIPTextSemanticEncoder(cfg, cfg["device"]) if self.use_clip_semantic_alignment else None
...
def main():
    ...
    if getattr(model, "text_encoder", None) is not None:
        print(f"[clip] backend={model.text_encoder.backend}, text_dim={model.text_encoder.out_dim}")
        if cfg.get("clip_local_weight_path"):

...
# ============================================================
# M3: RGB + Skeleton + Perception Alignment + CLIP Semantic Alignment
...
CONFIG = {
    ...
    "experiment_name": "m3_clip",
    "output_dir": r"E:\CE-CSL\CE-CSL\experiments\m3_clip",
...
class M3Model(nn.Module):
    ...
    def __init__(self,vocab_size,cfg):
        ...
        self.clip_proj=nn.Linear(cfg["fusion_dim"],cfg["clip_dim"])
    ...
    def forward(self,frames,skeleton):
        ...
        clip_feat=self.clip_proj(x.mean(dim=1))

        return F.log_softmax(logits,dim=-1),clip_feat
...
def train():
    ...
    model=M3Model(len(vocab),cfg).to(device)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for i,batch in enumerate(train_loader):
            ...
            log_probs,clip_feat=model(frames,skeleton)
            ...
            texts=[" ".join(g) for g in batch["gloss"]]

            text_tokens=clip.tokenize(texts).to(device)

            text_features=clip_model.encode_text(text_tokens)

            clip_loss=F.mse_loss(clip_feat,text_features)

            loss=loss_ctc+cfg["clip_weight"]*clip_loss
        ...
        with torch.no_grad():
            ...
            for batch in dev_loader:
                ...
                log_probs,_=model(frames,skeleton)

# -*- coding: utf-8 -*-
"""
M2: RGB + ST-GCN Skeleton + Perception Alignment for CE-CSL
...
CONFIG = {
    ...
    "experiment_name": "m2_rgb_skel_align_stgcn",
    "output_dir": f"{DATA_ROOT}/experiments/m2_rgb_skel_align_stgcn",
    "seed": 42,
...
def cosine_alignment_loss(rgb_feat: torch.Tensor, sk_feat: torch.Tensor, valid_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
    rgb_feat = F.normalize(rgb_feat, dim=-1)
    sk_feat = F.normalize(sk_feat, dim=-1)
    loss = 1.0 - (rgb_feat * sk_feat).sum(dim=-1)
...
class M2RGBSkeletonAlignCTC(nn.Module):
...
def train_one_epoch(model, loader, criterion, optimizer, device, blank_id, grad_clip, align_loss_weight: float = 0.0, log_interval=20) -> EpochResult:
    model.train()
    t0 = time.time()
    total_loss = 0.0
    total_batches = 0
    ...
    for bi, batch in enumerate(loader):
        ...
        outputs = model(frames, skeleton, input_lengths)
        ...
        loss = ctc_loss + align_loss_weight * align_loss
        loss.backward()
        ...
        if (bi + 1) % log_interval == 0:
            print(
                ...
                f"ctc_loss={ctc_loss.item():.4f} align_loss={align_loss.item():.4f}"
            )
    wer = compute_wer(all_refs, all_hyps)
    metrics = compute_token_metrics(all_refs, all_hyps)
    result = EpochResult(total_loss / max(total_batches, 1), wer, metrics["accuracy"], metrics["precision"], metrics["recall"], metrics["f1"], time.time() - t0)
    result.ctc_loss = total_ctc_loss / max(total_batches, 1)
    result.align_loss = total_align_loss / max(total_batches, 1)
    return result


@torch.no_grad()
def evaluate(model, loader, criterion, device, blank_id, align_loss_weight: float = 0.0) -> EpochResult:
    model.eval()
    t0 = time.time()
    total_loss = 0.0
    total_batches = 0
...
def main():
    ...
    model = M2RGBSkeletonAlignCTC(vocab.size, cfg).to(device)
    ...
    for epoch in range(start_epoch, cfg["epochs"]):
        ...
        train_res = train_one_epoch(model, train_loader, criterion, optimizer, device, vocab.blank_id, cfg["grad_clip"], cfg.get("align_loss_weight", 0.0), cfg["log_interval"])
        dev_res = evaluate(model, dev_loader, criterion, device, vocab.blank_id, cfg.get("align_loss_weight", 0.0))
        ...
        row = {
            ...
            "train_ctc_loss": getattr(train_res, "ctc_loss", train_res.loss),
            "train_align_loss": getattr(train_res, "align_loss", 0.0),
            ...
            "dev_ctc_loss": getattr(dev_res, "ctc_loss", dev_res.loss),
            "dev_align_loss": getattr(dev_res, "align_loss", 0.0),
            "dev_wer": dev_res.wer,
        ...
        print(
            ...
            f"(ctc={getattr(train_res, 'ctc_loss', train_res.loss):.4f}, align={getattr(train_res, 'align_loss', 0.0):.4f}) "
            f"train_WER={train_res.wer:.4f} | dev_loss={dev_res.loss:.4f} "
            f"(ctc={getattr(dev_res, 'ctc_loss', dev_res.loss):.4f}, align={getattr(dev_res, 'align_loss', 0.0):.4f}) "
    ...
    if cfg["test_after_training"]:
        ...
        test_res = evaluate(model, test_loader, criterion, device, vocab.blank_id, cfg.get("align_loss_weight", 0.0))
        print("\n" + "=" * 80)
        print("[TEST RESULT]")
        ...
        save_json(
            {
                ...
                "test_ctc_loss": getattr(test_res, "ctc_loss", test_res.loss),
                "test_align_loss": getattr(test_res, "align_loss", 0.0),
